...
var polar_spawns: Dictionary = {}
var polar_spawn := "under"
var activate_carrier_after_switch := false
...
var carrier_driver: Node
var simulation_clock:=preload("res://hub/simulation_clock.gd").new()
...
func _ready() -> void:
	...
	if options.has("leviathan"):
		if not OS.has_feature("double"):
			push_error("This Leviathan development scene requires the validated double engine.");get_tree().quit(2);return
		profiles.leviathan=JSON.parse_string(FileAccess.get_file_as_string("res://leviathan/host_physics_profile.json"))
		polar_spawns=JSON.parse_string(FileAccess.get_file_as_string("res://polar_range/spawns.json"))
		polar_spawn=str(options.get("polar_spawn","under"))
		if not polar_spawns.has(polar_spawn):polar_spawn="under"
	...
	atelier = load("res://polar_range/scene.gd" if is_polar_range() else "res://science_station/station.gd" if is_science_station() else "res://atelier/workshop.gd").new()
	add_child(atelier)
	atelier.build(self)
	...
	if is_science_station():
		...
		if options.plan.has("initial_position"):
			...
			if options.plan.has("initial_height_m"):probe_spawn.y=float(options.plan.initial_height_m)
...
func is_polar_range() -> bool:
	return options.get("scene", "workshop")=="polar_range"

func is_science_station() -> bool:
	return options.get("scene","workshop") in ["science_station","polar_range"]

func scene_title() -> String:
	return "03 · 极地雪原" if is_polar_range() else StationLayout.TITLE if is_science_station() else "小小维修站"

func spawn_point() -> Vector3:
	if probe_spawn != null:return probe_spawn
	if is_polar_range():
		var spec:Dictionary=polar_spawns[polar_spawn]
		var p:Array=spec.position
		var local:=Vector3(p[0],p[1],p[2])
		if spec.frame=="world" or leviathan==null:return local
		return leviathan.vehicle.runtime.get_design_transform(str(spec.frame))*local
	return StationLayout.CHECKPOINTS[checkpoint].position if is_science_station() else Vector3.ZERO

func polar_support_height() -> float:
	return spawn_point().y if is_polar_range() and polar_spawn!="under" else atelier.ground_height(_base.global_position.x,_base.global_position.z)

func select_polar_spawn(location:String) -> void:
	if not is_polar_range() or switching or not polar_spawns.has(location):return
	polar_spawn=location
	probe_spawn=null
	if driving_leviathan():carrier_driver.select(false)
...
func select_robot(kind: String) -> void:
	...
	if kind=="leviathan":
		if active_robot!="sai":
			activate_carrier_after_switch=true
			kind="sai"
		elif carrier_driver!=null:
			carrier_driver.select(true)
			return
	if kind=="sai" and carrier_driver!=null and carrier_driver.selected:
		carrier_driver.select(false)
		return
	if kind not in ["microduck","roller","sai"]:return
...
func _change_robot(kind: String) -> void:
	if carrier_driver!=null:carrier_driver.select(false)
	...
	if leviathan!=null and not options.plan.is_empty():before.carrier_before=leviathan.vehicle.runtime.state()
	...
	var loading_carrier:=options.has("leviathan") and leviathan==null
	var profile:Dictionary=profiles.leviathan if loading_carrier or (options.has("leviathan") and kind=="sai") else profiles.sai if kind=="sai" else profiles.microduck
	_apply_physics_profile(profile,2000 if loading_carrier or kind=="sai" else 200,100 if loading_carrier or kind=="sai" else 32)
	var previous_world: World3D = get_world_3d()
	if not is_polar_range():get_viewport().world_3d = World3D.new()
	if options.has("leviathan") and leviathan==null:
		leviathan=load("res://hub/leviathan.gd").new();add_child(leviathan)
		if not leviathan.load_vehicle(self,options.leviathan,"res://leviathan",not _headless):
			push_error(leviathan.failure);get_tree().quit(2);return
		leviathan.vehicle.runtime.use_native_jel=true
		if options.get("native_batch",false):
			leviathan.vehicle.runtime.use_native_cargo=true
			leviathan.vehicle.runtime.use_native_wheels=true
			leviathan.vehicle.runtime.use_native_jel_bodies=true
			leviathan.vehicle.runtime.use_native_jel_compact=true
		leviathan.vehicle.runtime.include_body_poses=not options.plan.is_empty()
		carrier_driver=load("res://hub/leviathan_driver.gd").new();carrier_driver.hub=self;add_child(carrier_driver)
	if leviathan!=null:
		leviathan.set_parked(kind!="sai")
		if kind!="sai":_apply_physics_profile(profiles.microduck,200,32)
	if leviathan!=null and not options.plan.is_empty():before.carrier_after=leviathan.vehicle.runtime.state()
	...
	if kind == "sai":
		...
		add_child(actor)
		if leviathan!=null:leviathan.adapt_robot(actor.robot)
		...
		if leviathan!=null:leviathan.adapt_robot(actor.get_node("RobotHost"))
		if not _headless:
	...
	if is_polar_range():
		if kind=="sai":simulation_clock.set_rate(float(options.get("time_scale",1.)))
		else:
			simulation_clock.rate=1.
			Engine.time_scale=1.
			Engine.physics_ticks_per_second=200
	switching = false
	if activate_carrier_after_switch and carrier_driver!=null:
		activate_carrier_after_switch=false
		carrier_driver.select(true)
...
func _apply_physics_profile(profile:Dictionary,hz:int,max_steps:int) -> void:
	for key in profile:
...
func driving_leviathan() -> bool:
	return carrier_driver!=null and carrier_driver.selected
...
func _make_hud() -> void:
	...
	panel.add_child(row)
	for kind in (["microduck","roller","sai","leviathan"] if options.has("leviathan") else ["microduck","roller","sai"]):
		var button := Button.new()
		button.text = {"microduck":"F5 · MicroDuck","roller":"F6 · MD 轮滑","sai":"F7 · Sai 001","leviathan":"F8 · Leviathan"}[kind]
	...
	if is_polar_range():
		var spawn_row:=HBoxContainer.new();panel.add_child(spawn_row)
		for location in ["under","platform","cockpit"]:
			var spawn_button:=Button.new()
			spawn_button.text={"under":"1 · 车底","platform":"2 · 平台上","cockpit":"3 · 驾驶舱室内"}[location]
			spawn_button.focus_mode=Control.FOCUS_NONE
			spawn_button.pressed.connect(select_polar_spawn.bind(location))
			spawn_row.add_child(spawn_button)
		var clock_row:=HBoxContainer.new();panel.add_child(clock_row)
		time_label=Label.new();clock_row.add_child(time_label)
		time_label.add_theme_color_override("font_color",Color("292830"))
		time_label.add_theme_color_override("font_outline_color",Color("ece8df"))
		time_label.add_theme_constant_override("outline_size",4)
		time_label.custom_minimum_size.x=146
		time_slider=HSlider.new();time_slider.name="TimeScaleSlider"
		time_slider.min_value=.1;time_slider.max_value=3.;time_slider.step=.1
		time_slider.value=float(options.get("time_scale",1.))
		time_slider.custom_minimum_size=Vector2(220,30)
		time_slider.focus_mode=Control.FOCUS_NONE
		time_slider.value_changed.connect(_set_time_rate)
		clock_row.add_child(time_slider)
		var reset_clock:=Button.new();reset_clock.text="1×"
		reset_clock.focus_mode=Control.FOCUS_NONE
		reset_clock.pressed.connect(func():time_slider.value=1.)
		clock_row.add_child(reset_clock)
		time_label.text="时间流速 %.1f×"%time_slider.value
...
func _refresh_controls() -> void:
	...
	if driving_leviathan():
		controls_hint.text=carrier_driver.controls_text()
		grab_row.visible=false
		for kind in robot_buttons:robot_buttons[kind].set_pressed_no_signal(kind=="leviathan")
		return
	...
	if is_polar_range():controls_hint.text="F5 MicroDuck / F6 轮滑版 / F7 Sai / F8 利维坦 · W/S 前后 · A/D 转向\n1 车底 / 2 平台上 / 3 驾驶舱室内 · R 复位 · 右键视角 · 滚轮缩放 · Esc 退出"
	if time_slider!=null:time_slider.editable=active_robot=="sai"
	...
	grab_row.visible = actor!=null and active_robot == "sai" and actor.grab!=null and active_task in ["drive", "sort"] and not finished
...
func _grab_action(action: String) -> void:
	if driving_leviathan() or switching or stopping or active_robot != "sai" or actor == null or actor.grab == null or actor.finished: return
	actor.grab.perform(action)

func _input(event: InputEvent) -> void:
	if event is InputEventKey and options.get("plan",{}).get("trace_keys",false):
		input_trace.append({"time":elapsed,"key":event.keycode,"physical_key":event.physical_keycode,"pressed":event.pressed,"focused":get_window().has_focus()})
	if atelier==null:
		if event is InputEventKey and event.pressed and event.physical_keycode==KEY_ESCAPE:get_tree().quit()
		return
	if carrier_driver!=null:carrier_driver.handle(event)
	if event is InputEventKey and event.pressed and not event.echo:
		if event.physical_keycode==KEY_TAB:
			if driving_leviathan():carrier_driver.view_index=1-carrier_driver.view_index
			elif is_science_station():atelier.cycle_view()
			else:atelier.set_view("tour" if atelier.view=="follow" else "follow")
			get_viewport().set_input_as_handled()
		elif event.physical_keycode in [KEY_F5,KEY_F6,KEY_F7,KEY_F8]:
			select_robot({KEY_F5:"microduck",KEY_F6:"roller",KEY_F7:"sai",KEY_F8:"leviathan"}[event.physical_keycode])
			get_viewport().set_input_as_handled()
		elif is_polar_range() and event.physical_keycode in [KEY_1,KEY_2,KEY_3]:
			select_polar_spawn({KEY_1:"under",KEY_2:"platform",KEY_3:"cockpit"}[event.physical_keycode])
			get_viewport().set_input_as_handled()
		elif not driving_leviathan() and (event.physical_keycode == KEY_0 or (event.physical_keycode == KEY_R and active_robot == "sai")):
	...
	if event is InputEventMouseButton:
		...
		if event.pressed and event.button_index in [MOUSE_BUTTON_WHEEL_UP,MOUSE_BUTTON_WHEEL_DOWN]:
			if driving_leviathan():
				carrier_driver.distance=clampf(carrier_driver.distance*(.9 if event.button_index==MOUSE_BUTTON_WHEEL_UP else 1.1),65.,600.)
				return
	...
	if event is InputEventMouseMotion and drag:
		if driving_leviathan():
			carrier_driver.orbit_yaw-=event.relative.x*.006
			carrier_driver.orbit_pitch=clampf(carrier_driver.orbit_pitch+event.relative.y*.004,.05,1.35)
			return
...
func _physics_process(delta: float) -> void:
	...
	if is_science_station() and not is_polar_range():
	...
	if elapsed >= next_sample:
		...
		samples.append({"time":elapsed,"robot_time":_t,"robot":active_robot,"task":active_task,
			...
			"time_scale":simulation_clock.rate,"physics_delta":delta,
		...
		if carrier_driver!=null:samples[-1]["carrier"]=carrier_driver.telemetry()
	...
	while plan_index < actions.size() and elapsed >= float(actions[plan_index].at):
		...
		plan_index += 1
		if action.has("time_scale"):
			if time_slider!=null:time_slider.value=float(action.time_scale)
			else:_set_time_rate(float(action.time_scale))
		if action.has("carrier_command") and leviathan!=null:
			leviathan.vehicle.runtime.set_vehicle_command(float(action.carrier_command[0]),float(action.carrier_command[1]))
		if action.has("robot"): select_robot(action.robot)
		if action.has("spawn"):select_polar_spawn(action.spawn)
...
func _process(delta: float) -> void:
	...
	if options.plan.get("wall_seconds",0.)>0 and capture_start>0:
		if float(Time.get_ticks_usec()-capture_start)/1e6>=float(options.plan.wall_seconds):_finish.call_deferred()
	if not _headless:
		if driving_leviathan():carrier_driver.update_camera(delta)
		else:
			get_node("World/Camera3D").near=.015
		...
		if is_science_station() and not is_polar_range():label.text+=" · "+StationLayout.CHECKPOINTS[checkpoint].title
		if driving_leviathan():label.text=carrier_driver.status_text()
		_refresh_controls()
		...
		if options.plan.get("snapshot_at_wall_s",0.)>0 and not snapshot_saved and capture_start>0:
			if float(Time.get_ticks_usec()-capture_start)/1e6>=float(options.plan.snapshot_at_wall_s):
				snapshot_saved=true
				get_viewport().get_texture().get_image().save_png(options.output+"/view.png")
...
func _finish() -> void:
	if stopping: return
	stopping = true
	if carrier_driver!=null:carrier_driver.stop()
	if atelier == null:
		get_tree().quit();return
	_save_native_trace()
	if capture_job >= 0: WorkerThreadPool.wait_for_task_completion(capture_job)
	var result := {"pid":OS.get_process_id(),"hub":get_instance_id(),"atelier":atelier.get_instance_id(),
		"events":events,"input_trace":input_trace,"samples":samples,"seconds":elapsed,"frames":frames,"dropped":dropped,"active_robot":active_robot,
		"task":active_task,"task_success":task_result.get("success",null)}
	if carrier_driver!=null:result["carrier"]=carrier_driver.telemetry()
...
func _set_time_rate(value:float) -> void:
	if active_robot!="sai":
		simulation_clock.rate=1.
		Engine.time_scale=1.
		Engine.physics_ticks_per_second=200
		if time_label!=null:time_label.text="时间流速 1.0× · MicroDuck 固定"
		return
	options.time_scale=value
	simulation_clock.set_rate(value)
	if time_label!=null:time_label.text="时间流速 %.1f×"%simulation_clock.rate
