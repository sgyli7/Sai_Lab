...
func observation() -> Dictionary:
	...
	if scene.hub.leviathan!=null:
		for other in body.get_colliding_bodies():
			if scene.hub.leviathan.vehicle.runtime.bodies.get(str(other.name))==other:
				support_id=str(other.name);break
	return {"serial":serial,"request":request,"target_m":scene.robot.source(center),"target_support_body_id":support_id,
