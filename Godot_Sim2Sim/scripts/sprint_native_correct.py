...
def main():
    ...
    p.add_argument('--retention-reference',choices=['s05','initial'],default='s05')
    a=p.parse_args()
    ...
    retention=WalkingTeacherRetention(policy,'replay_kl',TEACHER,384,
                                      reference_increment=policy.delta if a.retention_reference=='initial' else None).to('cuda')
    ...
    config=dict(seed=a.seed,iterations=a.iterations,seeds_per_batch=a.seeds_per_batch,curriculum=a.curriculum,
                retention_reference=a.retention_reference,
