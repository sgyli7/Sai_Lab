...
def prepare_sampler(canonical, project, instrumentation):
    ...
    driver = project / 'standalone/driver.gd'
    code = driver.read_text()
    substitutions = [
        ('var sample_std := 0.02', 'var sample_std := 0.02\nvar sample_after := 0.0'),
        ('\t\tif arg.begins_with("--sample-std="): sample_std=float(arg.trim_prefix("--sample-std="))',
         '\t\tif arg.begins_with("--sample-std="): sample_std=float(arg.trim_prefix("--sample-std="))\n\t\tif arg.begins_with("--sample-after="): sample_after=float(arg.trim_prefix("--sample-after="))'),
        ('\tif sample_std not in [0.0,0.02]:', '\tif not is_finite(sample_after) or sample_after<0.0 or sample_after>25.0:\n\t\t_fatal("Invalid exploration prefix")\n\t\treturn\n\tif sample_std not in [0.0,0.02]:'),
        ('\tvar action: PackedFloat32Array = bank[skill].infer(obs)',
         '\tif skill == "sprint": bank[skill].sigma = sample_std if elapsed+1e-9>=sample_after else 0.0\n\tvar action: PackedFloat32Array = bank[skill].infer(obs)'),
        ('"training_exploration":sample_std,"schema_version":1',
         '"training_exploration":sample_std,"training_exploration_after":sample_after,"schema_version":1'),
    ]
    for before, after in substitutions:
        if code.count(before) != 1:
            raise ValueError('Exploration instrumentation no longer matches its audited source')
...
def collect(project, actor, directory, seeds, noise_seed, *, sigma=.02, workers=4,
            prefix_seeds=(), explore_after=7., selected=None):
    project = Path(project).resolve()
    ...
    cases = write_cases(directory / 'cases', seeds, .3, paired=False, control=deployment['control_config'], selected=selected)
    for case in cases:
        value = json.loads(case.read_text())
        value['training_exploration'] = True
        value['training_exploration_after'] = explore_after if value['seed'] in prefix_seeds else 0.
    ...
    def run(index_case):
        ...
        after = json.loads(case.read_text())['training_exploration_after']
        try:
            with (folder / 'player.log').open('w') as log:
                subprocess.run(['godot', '--headless', '--fixed-fps', '200', '--path', str(overlay),
                                ...
                                '--seed=' + str(noise_seed + i), '--sample-after=' + str(after)], stdout=log, stderr=subprocess.STDOUT,
        ...
        assert summary['training_exploration_after'] == after
        ...
        value = dict(case=str(case), trace=str(folder / 'trace.json'), steps=len(trace['rows']),
                     sigma=sigma, explore_after=after, noise_seed=noise_seed + i, first_fall=summary['first_fall'],
...
def dataset(rollouts, deployment, device='cuda'):
    ...
    exploration = np.zeros(shape, bool)
    program = []
    last_error = 0.
    final_time_error = 0.
    for n, e in enumerate(entries):
        ...
        after = e.get('explore_after',0.)
        assert trace['summary'].get('training_exploration_after',0.) == after
        ...
        for t, row in enumerate(trace['rows']):
            ...
            exploration[t,n] = sprint[t,n] and row['t']+1e-9>=after
    ...
    data.update(cobs=torch.stack(cobs),reward=torch.stack(rewards),cost=torch.stack(costs).float(),
                done=done,valid=valid,sprint=selection,mask=torch.as_tensor(exploration,device=device)&valid)
    data['reward'][~valid]=0.
    data['cost'][~valid]=0.
    audit = dict(episodes=count,rows=int(valid.sum()),actor_rows=int(data['mask'].sum()),
                 ...
                 falls=int(fall.any(0).sum()),deterministic_prefix_sprint_rows=int((selection&valid&~data['mask']).sum()),
                 command_max_abs=error,last_action_max_abs=last_error,
