...
def add_joint_compliance(spec, godot_spec, timeconst):
    ...
    result = _add_translations(spec, godot_spec, [timeconst, 1.])
    result.update(timeconst=timeconst, solref_format='timeconst')
    return result


def add_joint_compliance_direct(spec, godot_spec, stiffness, damping):
    stiffness, damping = float(stiffness), float(damping)
    if not all(math.isfinite(x) and x > 0 for x in [stiffness, damping]):
        raise ValueError('Direct equality stiffness/damping must be finite and positive')
    result = _add_translations(spec, godot_spec, [-stiffness, -damping])
    result.update(stiffness=stiffness, damping=damping, solref_format='direct')
    return result


def _add_translations(spec, godot_spec, solref):
    import mujoco
    ...
    for joint in source['joints']:
        ...
        for axis in range(3):
            ...
            vector[axis] = 1.
            body.add_joint(
                name=name, type=mujoco.mjtJoint.mjJNT_SLIDE, axis=vector,
                pos=[0., 0., 0.], limited=False, range=[0., 0.],
                damping=0., frictionloss=0., armature=0., stiffness=0., ref=0., springref=0.,
            )
            eq = spec.add_equality(
                name=name + '_zero', type=mujoco.mjtEq.mjEQ_JOINT,
                objtype=mujoco.mjtObj.mjOBJ_JOINT, name1=name, active=True,
                solref=solref, solimp=[.99, .99, .001, .5, 2.],
            )
            eq.data[:] = 0.
            names.append(name)
    if len(names) != 42:
        raise ValueError('Expected three passive translations for 14 hinges')
    return dict(
        mode='passive_translation_equalities_v1', solref=solref,
