...
func _regenerate_biomes_in_scene() -> void:
    # MicroDuck fork: editor biomes refresh is optional; avoid hard dep on class_name Biomes.
...
func _regenerate_biomes_recursive(node: Node) -> void:
    if node.has_method("regenerate") and node.get_script() != null:
        var sn := str(node.get_script().resource_path)
        if "Biomes.gd" in sn or "gnd_biomes" in sn:
            node.call("regenerate")
            return
