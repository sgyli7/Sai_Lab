...
recipe=json.loads((session/'r2_recipe.json').read_text())
# R0 already owns training resources; do not compete with its subsequent runs.
wait_started=time.monotonic()
while not (session/recipe['wait_for']).exists():
 if time.monotonic()-wait_started>5000:raise RuntimeError('R1 did not finish; R2 never started')
 time.sleep(5)
completed=[]
for seed in recipe['training_seeds']:
 for mode in recipe['actor_modes']:
  ...
  name=f'r2_{mode}_s{seed}';started=time.time()
  ...
  atomic_json(session/'r2_progress.json',completed);print('R2_RESULT '+json.dumps(result),flush=True)
  if not result['completed']:raise SystemExit(1)
atomic_json(session/'r2_completed.json',completed)
