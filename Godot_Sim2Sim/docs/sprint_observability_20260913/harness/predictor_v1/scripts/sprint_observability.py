...
def protocol():
    return dict(version=1, architecture=[WIDTH, 128, 128, 3], activation='SiLU',
                arms=list(ARMS), seeds=[997101, 997102], epochs=40, batch_size=4096,
                ...
                validation='Fixed epoch 10/20/30/40 learning curves only; never choose endpoint by validation/test')
...
def analyze(output):
    ...
    for seed in protocol()['seeds']:
        ...
        for arm in ARMS[1:]:
            ...
            for subset in ('moving','transition'):
                baseline = results[f'immediate_{seed}'][subset]
                ...
                checks[subset] = dict(rmse_reduction=float(reduction),component_regression=comp.tolist(),
    ...
    selected = [arm for arm in ARMS[1:] if all(d['passed'] for d in decisions if d['arm']==arm)]
    atomic_json(output/'analysis.json',dict(results=results,decisions=decisions,information_gate_passed=selected,
                test_reset_seeds=reset_seeds.tolist(),scope=protocol()['scope']))
...
def main():
    ...
    if args.stage == 'freeze':
        ...
        atomic_json(path,dict(protocol=protocol(),created_unix=time.time(),
