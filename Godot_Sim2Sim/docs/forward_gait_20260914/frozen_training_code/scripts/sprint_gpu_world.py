...
class GpuWorld:
    ...
    def enable_swing_monitor(self):
        """Read actual solver contacts without changing the physical model."""
        from sim2sim.research.support_swing import SupportSwing
        self.swing = SupportSwing(self.count, 'cuda')
        self.foot_geom_ids = [mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_GEOM, name)
                              for name in ['left_foot_collision', 'right_foot_collision']]
        if min(self.foot_geom_ids) < 0:raise ValueError('Missing foot collision geometry')
        if self.feet is None:raise ValueError('Swing monitor requires verified Jolt foot hulls')
        self.foot_bodies = [int(self.model.geom_bodyid[gid]) for gid in self.foot_geom_ids]
        self.foot_vertices = [torch.tensor(self.feet['feet'][name]['vertices_body'],device='cuda',dtype=torch.float32)
                              for name in ['left_foot_collision','right_foot_collision']]
        self.ground_geom = torch.tensor(self.model.geom_bodyid == 0, device='cuda')
        self.contact_indices = torch.arange(self.wd.naconmax, device='cuda')

    def foot_contacts(self):
        c = self.wd.contact
        geom = wp.to_torch(c.geom).long()
        valid = (self.contact_indices < wp.to_torch(self.wd.nacon)[0]) & (wp.to_torch(c.dist) < 0.) & (wp.to_torch(c.efc_address)[:, 0] >= 0)
        world = wp.to_torch(c.worldid).long().clamp(0, self.count - 1)
        ground = self.ground_geom[geom.clamp(0, self.model.ngeom - 1)]
        result = []
        for foot in self.foot_geom_ids:
            found = valid & (((geom[:, 0] == foot) & ground[:, 1]) | ((geom[:, 1] == foot) & ground[:, 0]))
            counts = torch.zeros(self.count, device='cuda', dtype=torch.int32)
            counts.scatter_add_(0, world, found.int())
            result.append(counts > 0)
        return torch.stack(result, dim=-1)

    def foot_clearance(self):
        # Frozen flat floor z=0; use the lowest actual hull vertex, not ankle z.
        positions, matrices = wp.to_torch(self.wd.xpos), wp.to_torch(self.wd.xmat)
        return torch.stack([positions[:,body,2] + (matrices[:,body,2,None,:]*vertices[None]).sum(-1).amin(-1)
                            for body,vertices in zip(self.foot_bodies,self.foot_vertices)],dim=-1)

    def reset(self, ids, perturbation=None):
        if self.swing is not None:self.swing.reset(ids)
        ...
        if self.observer is not None:
            self.observer.reset(ids, self.qpos[:, self.qi], self.qpos[:, self.qa + 3:self.qa + 7])
        self.forward()
    ...
    def step(self, action):
        ...
        with torch.cuda.stream(self.torch_stream), wp.ScopedStream(self.stream):
            ...
            for _ in range(4):
                ...
                if self.observer is not None:
                    self.observer.update(self.qpos[:, self.qi], self.qpos[:, self.qa + 3:self.qa + 7])
                if self.swing is not None:self.swing.update(self.foot_contacts(),self.foot_clearance())
