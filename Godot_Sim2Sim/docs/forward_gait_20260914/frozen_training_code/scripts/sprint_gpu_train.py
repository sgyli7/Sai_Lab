...
def main():
 ...
 p.add_argument('--control',type=Path,default=Path('results/sprint_joint_20260912/delivery/control.json'))
 p.add_argument('--initialize-checkpoint',type=Path,help='Warm actor/critic only; fresh optimizer, RNG and physical episodes')
 p.add_argument('--gradient-audit',action='store_true')
 p.add_argument('--swing-objective',choices=['off','support'],default='off')
 p.add_argument('--monitor-swing',action='store_true')
 ...
 if a.initialize_checkpoint and a.resume:raise ValueError('Choose warm initialization or exact resume')
 control=a.control;settings=json.loads(control.read_text())['walk']
 ...
 initialization=None
 if a.initialize_checkpoint:
  initial=torch.load(a.initialize_checkpoint,map_location='cuda',weights_only=False)
  for key,expected in [('source_sha256',anchor.sha256),('template_sha256',hashlib.sha256(template.read_bytes()).hexdigest()),('variant','residual'),('std',.02),('bound',.2),('velocity_observer',a.velocity_observer),('controller',a.controller)]:
   if initial['config'][key]!=expected:raise ValueError('Warm initialization mismatch: '+key)
  policy.load_state_dict(initial['policy']);critic.load_state_dict(initial['critic'])
  initialization=dict(path=str(a.initialize_checkpoint),sha256=hashlib.sha256(a.initialize_checkpoint.read_bytes()).hexdigest(),iteration=initial['iteration'],scope='actor and critic weights only; fresh optimizer/RNG/physical episodes')
 ao=torch.optim.Adam(policy.delta.parameters(),lr=1e-4);co=torch.optim.Adam(critic.parameters(),lr=3e-4)
 world=GpuWorld(a.envs,settings,feet=a.feet,graphs=a.cuda_graphs,velocity_observer=a.velocity_observer)
 if a.monitor_swing or a.swing_objective!='off':world.enable_swing_monitor()
 ...
 def reward(requested,command,action,previous):
  ...
  detail=dict(progress=progression.mean(),tracking=tracking.mean(),balance=balance.mean(),smooth=smooth.mean())
  if world.swing is not None:
   swing_reward=world.swing.reward(requested,speed,tilt,pos[:,2],sprint)
   if a.swing_objective=='support':value=value+1.5*swing_reward
   detail.update(swing=swing_reward.mean(),single_support=(world.swing.contact.sum(-1)==1).float().mean())
  return value,fell,detail
 files=[Path(__file__),Path('scripts/sprint_gpu_world.py'),Path('scripts/sprint_gpu_proxy.py'),Path('src/sim2sim/research/torch_anchor.py'),Path('src/sim2sim/research/torch_walking.py'),Path('src/sim2sim/research/models.py'),Path('src/sim2sim/research/constraint_returns.py'),Path('src/sim2sim/research/sprint_constraints.py'),Path('src/sim2sim/research/walking_retention.py'),Path('src/sim2sim/research/locomotion_objectives.py'),Path('src/sim2sim/research/sprint_courses.py'),Path('src/sim2sim/research/kinematic_observer.py')]
 files=[f if f.is_absolute() else Path(__file__).resolve().parents[1]/f for f in files]
 ...
 try:
  ...
  for iteration in range(initial_iteration+1,a.iterations+1):
   ...
   torch.cuda.synchronize();collection_s=time.monotonic()-collect_start;learn_start=time.monotonic();before=copy.deepcopy(policy.state_dict());before_optimizer=copy.deepcopy(ao.state_dict());updates=0;stop=False;teacher_losses=[];gradient_audit=None
   for epoch in range(4):
    for ix in torch.randperm(a.steps*a.envs,device='cuda').split(4096):
     ...
     ppo_loss=-torch.minimum(ratio*adv[ix],ratio.clamp(.8,1.2)*adv[ix]).mean()
     loss=ppo_loss+.02*policy.delta(flat['obs'][ix]).square().mean()
     if retention:
      ...
      if a.gradient_audit and gradient_audit is None and iteration%16==0:
       parameters=[v for v in policy.delta.parameters() if v.requires_grad]
       gp=torch.autograd.grad(ppo_loss,parameters,retain_graph=True)
       gt=torch.autograd.grad(a.teacher_weight*teacher_loss,parameters,retain_graph=True)
       dot=sum((x*y).sum() for x,y in zip(gp,gt));pn=sum(x.square().sum() for x in gp).sqrt();tn=sum(x.square().sum() for x in gt).sqrt()
       gradient_audit=dict(ppo_norm=float(pn),weighted_teacher_norm=float(tn),cosine=float(dot/(pn*tn+1e-12)),meaning='same sampled PPO minibatch versus replay teacher gradient before clipping; negative cosine indicates local conflict')
