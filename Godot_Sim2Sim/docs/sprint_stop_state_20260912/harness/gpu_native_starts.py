...
sys.path.insert(0,str(Path('results/sprint_joint_gpu_20260912').resolve()))
...
p=argparse.ArgumentParser();p.add_argument('--actor',type=Path,required=True);p.add_argument('--ordinary',type=Path,required=True);p.add_argument('--feet',choices=['source','jolt'],required=True);p.add_argument('--velocity-observer',choices=['solver','joint_fd'],default='solver');p.add_argument('--out',type=Path,required=True);a=p.parse_args()
...
world=GpuWorld(n,settings,feet=a.feet,graphs=True,velocity_observer=a.velocity_observer);ordinary=TorchAnchor(source).cuda().eval();actor=TorchAnchor(a.actor).cuda().eval()
...
states=torch.empty((1251,n,13),device='cuda');actions=torch.empty((1250,n,14),device='cuda');observations=torch.empty((1250,n,61),device='cuda');commands=torch.empty_like(requested)
try:
 ...
 pos,q,ang,v=world.state();states[-1]=torch.cat([pos,q,ang,v],dim=1)
 ...
 for j,case in enumerate(cases):
  results.append(dict(case=case['case'],seed=case['seed'],ordinary=case.get('ordinary_control',False),metrics=score_samples(data[:,j],case,tapes[j],selections[j])))
 candidates=[x for x in results if not x['ordinary']]
 atomic_json(a.out/'completed.json',dict(status='completed',velocity_observer=a.velocity_observer,actor_sha256=actor.sha256,ordinary_sha256=ordinary.sha256,feet=a.feet,worlds=n,elapsed_rollout_s=elapsed,parity_max_abs=parity,passes=sum(x['metrics']['success'] for x in candidates),falls=sum(x['metrics']['fell'] for x in candidates),candidate_count=len(candidates),results=results,sampling='post_action_with_final_state_v2',scope='Projected native zero-velocity reset and identical input tapes; different contact solver, no native acceptance substitution'))
