...
root=Path('results/sprint_exit_20260912').resolve();budget=json.loads((root/'active_budget.json').read_text());events=budget['events']
...
result=dict(created_unix=time.time(),owned_live_processes=processes,owned_live_supervisors=supervisors,owned_running_containers=containers,
 jobs_started=len(starts),jobs_finished=len(complete_events),actors=list(budget['actors']),passed=not(processes or supervisors or containers),
