...
r=Path('results/sprint_exit_20260912');out=r/'speed_budget_probe';out.mkdir(exist_ok=False)
...
for source in [r/'minimize/remove_forward_prefix.json',*sorted((r/'baseline_exits/cases').glob('*.json'))]:
 case=json.loads(source.read_text());case['control_config']['walk']['walk_path_speed_budget']=True
