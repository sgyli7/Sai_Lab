...
def main():
    ...
    for name,contact,strong_impedance in profiles:
        ...
        try:
            ...
            parity=max(float((w.qpos-expected_q).abs().max()),float((w.qvel-expected_v).abs().max()),float((w.observer.qd-expected_qd).abs().max()))
            if parity>1e-7:raise RuntimeError('Microstep subdivision changed dynamics')
            initialize(w,rows)
            ...
            values=dict(microstep_parity_max_abs=parity,metrics=metric,lag_probe_velocity_rmse=lag,
