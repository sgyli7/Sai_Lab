...
def main():
    ...
    out=R/'compliance_direct_probe';out.mkdir(exist_ok=False)
    ...
    profiles=[('rigid','source',None),('direct20k','source',(20000.,200.)),('direct20k_hard_contact','two_tick',(20000.,200.)),
              ('direct30k','source',(30000.,200.)),('direct30k_hard_contact','two_tick',(30000.,200.))]
    protocol=dict(profiles=profiles,seeds=[e['seed'] for e in selected],train_seed_count=8,validation_seed_count=8,
        ...
        rationale='10ms positive-solref equalities reduced landing manifold error but overestimated standing/start deflection. Fixed direct damping 200 with stiffness 20000/30000 tests approximately 2x/3x stiffness without increasing damping. All four predeclared profiles use the same physical response gates.',
