...
def main():
    ...
    protocol=dict(profiles=profiles,velocity_observer='joint_fd',physics_step=.005,policy_step=.02,
        ...
        microstep_scope='Robot base and joint coordinates. Whole-world position/velocity separately capped at 1e-7/5e-6 after normal-repeat strong-contact audit localized 3.74e-6 to remote ball DOF 23; no contact metric or policy threshold changed.',
    ...
    for name,contact,strong_impedance in profiles:
        ...
        try:
            ...
            whole_world_parity=dict(qpos=float((w.qpos-expected_q).abs().max()),qvel=float((w.qvel-expected_v).abs().max()))
            parity_components=dict(qpos=float((w.qpos[:,qi]-expected_q[:,qi]).abs().max()),qvel=float((w.qvel[:,vi]-expected_v[:,vi]).abs().max()),observer_qd=float((w.observer.qd-expected_qd).abs().max()))
            ...
            if whole_world_parity['qpos']>1e-7 or whole_world_parity['qvel']>5e-6:
                raise RuntimeError('Whole-world canary exceeds measured repeatability')
            initialize(w,rows)
            ...
            values=dict(microstep_parity_max_abs=parity,microstep_parity_components=parity_components,whole_world_parity=whole_world_parity,metrics=metric,lag_probe_velocity_rmse=lag,
