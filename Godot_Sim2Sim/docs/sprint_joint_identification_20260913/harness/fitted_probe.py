...
def main():
    ...
    out=R/'fitted_physics';out.mkdir(exist_ok=False)
    ...
    fitted=json.loads((R/'fit_completed.json').read_text())
    assert fitted['eligible_for_single_physical_probe']
    profiles=[('rigid','source',None),('uniform20k','two_tick',(20000.,200.)),('fitted','two_tick',(20000.,200.))]
    protocol=dict(profiles=profiles,seeds=[e['seed'] for e in selected],train_seed_count=8,validation_seed_count=8,
        ...
        rationale='Single fitted per-hinge profile from first eight seeds landing/standing translation vectors; damping stays 200, contact stays two_tick. Re-run rigid and uniform controls. No selection based on final seeds or weakening the earlier physical/manifold gate.',
        fitted_profile=fitted['profile'],fit_sha256=hashlib.sha256((R/'fit_completed.json').read_bytes()).hexdigest(),
    ...
    for name,contact,compliance in profiles:
        ...
        # A local research override at the pre-compilation seam. Production
        # GpuWorld and the game's physical sources are unchanged.
        from sim2sim.research.joint_compliance import add_joint_compliance_direct
        def fitted_connections(spec,godot_spec,stiffness,damping):
            result=add_joint_compliance_direct(spec,godot_spec,stiffness,damping)
            for hinge,(k,d) in fitted['profile'].items():
                for axis in range(3):
                    spec.equality(f'proxy_compliance_{hinge}_{axis}_zero').solref=[-k,-d]
            result.pop('solref');result.pop('stiffness');result.pop('damping')
            result.update(solref_format='per_hinge_direct',profile=fitted['profile'])
            return result
        context=patch('sim2sim.research.joint_compliance.add_joint_compliance_direct',fitted_connections) if name=='fitted' else nullcontext()
        with context:
            w=GpuWorld(16,settings,contact=contact,feet='jolt',graphs=True,velocity_observer='joint_fd',joint_compliance_direct=compliance)
        if name=='fitted':
            for hinge,(k,d) in fitted['profile'].items():
                for axis in range(3):
                    eid=mujoco.mj_name2id(w.model,mujoco.mjtObj.mjOBJ_EQUALITY,f'proxy_compliance_{hinge}_{axis}_zero')
                    np.testing.assert_array_equal(w.model.eq_solref[eid],[-k,-d])
        try:
            initialize(w,rows)
