...
def main():
    ...
    out=R/'momentum_split_physics';out.mkdir(exist_ok=False)
    ...
    profiles=[('rigid','source',None),('uniform20k','two_tick',(20000.,200.)),('hard_velocity','two_tick',(1.,200.)),('momentum_split','two_tick',(1.,200.))]
    protocol=dict(profiles=profiles,seeds=[e['seed'] for e in selected],train_seed_count=8,validation_seed_count=8,
        ...
        rationale='Single follow-up to the incomplete split prototype: hard mass-weighted velocity projection sets slide rates to zero, then integrate exactly once from saved start pose; with/without alpha=.5775 position correction. K=0,b=200 and contact two_tick unchanged. Uses start-of-step mass, simultaneous projection, no contact split correction; not a full Jolt solver. All physical/manifold gates unchanged.',
        ...
        projection_canary='mass abs<1e-6, pose integration vs CPU double abs<2.5e-7 (diagnosed root quaternion difference 1.85 float32 ULPs), slide correction abs<1e-8, generalized velocity unchanged exactly; original physical/action gates untouched',
    ...
    for name,contact,compliance in profiles:
        ...
        from split_proxy import MomentumSplitWorld
        special=name in ['hard_velocity','momentum_split']
        ...
        with context:
            w=(MomentumSplitWorld if special else GpuWorld)(16,settings,contact=contact,feet='jolt',graphs=True,velocity_observer='joint_fd',joint_compliance_direct=compliance)
        if special:w.prepare_projection(1-(1-.35)**2 if name=='momentum_split' else 0.)
        try:
            initialize(w,rows)
            if special and name=='momentum_split':
                atomic_json(out/'projection_canary.json',w.canary())
                atomic_json(out/'velocity_canary.json',w.velocity_canary())
                initialize(w,rows)
