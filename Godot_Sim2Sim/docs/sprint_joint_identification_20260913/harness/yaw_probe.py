...
sys.path.insert(0,str(R));from yaw_reversal import YawReversalRamp

original_factory=replay.bank_and_brain
def reference_factory(config,mode,control_config=None,project=None):
 bank,brain=original_factory(config,mode,control_config,project)
 brain.ramp=YawReversalRamp(brain,(control_config or {}).get(mode,{}).get('sprint_yaw_reversal_s',0.))
 return bank,brain
# One binding before worker threads start, not nested global patches per thread.
replay.bank_and_brain=reference_factory
...
def run(entry,duration):
 ...
 try:
  ...
  shutil.copy2(R/'yaw_reversal.gd',overlay/'standalone/play_brain.gd')
  p=overlay/'standalone/driver.gd';text=p.read_text();needle='\tbrain.configure(flags,limits)\n';assert text.count(needle)==1
  p.write_text(text.replace(needle,needle+'\tbrain.reversal_seconds=float(control_config.get(session.mode,{}).get("sprint_yaw_reversal_s",0.))\n'))
 ...
 shadow=replay.shadow(folder/'trace.json',PRIOR/'suite/runtime')
 atomic_json(folder/'shadow.json',shadow)
 assert shadow['passed'],shadow
 changed=float(np.max(np.abs(np.asarray([x['command'] for x in old[300:310]])-np.asarray([x['command'] for x in new[300:310]]))))
 if duration>0:assert changed>.1,changed
 return dict(seed=entry['seed'],duration=duration,directory=str(folder),prefix_max_abs=maxima,intervention_max_abs=changed,task=result['task_metrics'],shadow=shadow)
...
def main():
 ...
 atomic_json(out/'protocol.json',dict(seeds=[e['seed'] for e in entries],duration_seconds=.2,scope='Input-layer control change only; a402 policy and game physics frozen. Existing angular ramp is ~80ms. During sprint reversal only, rate becomes 2*sprint_vmax_ang/.2; zero targets, ordinary behavior, translational ramps and cruise target remain original. No second motion filter.',gate='Both direction phases and stop/straight/fall gates in original full 18s case, all 16 dev seeds; disabled canary is bit-identical, active prefix before first reversal is bit-identical. Actual command change >.1 in first reversal window is mandatory. Python/native shadow must pass existing 1e-5 maximum error gate. No parameter sweep.',code_sha256={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),R/'yaw_reversal.py',R/'yaw_reversal.gd']}))
