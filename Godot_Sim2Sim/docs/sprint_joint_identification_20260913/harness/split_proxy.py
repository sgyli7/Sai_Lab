...
class SplitWorld(GpuWorld):
    ...
    def canary(self):
        ...
        actual=self.qpos[0].cpu().numpy();error=np.abs(actual-reference);index=int(np.argmax(error))
        self.canary_diagnostic=dict(index=index,actual=float(actual[index]),reference=float(reference[index]),float32_ulp=float(np.spacing(np.float32(abs(reference[index])))),error_ulps=float(error[index]/np.spacing(np.float32(abs(reference[index])))),mass_error=mass_error,integration_error=integration_error,slide_error=slide_error,velocity_error=velocity_error)
        print('PROJECTION_CANARY_DIAG',self.canary_diagnostic,flush=True)
        # The largest diagnosed difference is 1.85 float32 ULPs of the root quaternion.
        # This implementation canary is separate from policy action/physical gates.
        assert integration_error<2.5e-7 and slide_error<1e-8 and velocity_error==0.,self.canary_diagnostic
...
class MomentumSplitWorld(SplitWorld):
    """Add the missing hard velocity constraint before integrating pose.

    The first prototype retained finite equality damping, allowing relative
    slide motion. This variant projects momentum at the start-step mass matrix
    and re-integrates from the saved start pose, then corrects position drift.
    """
    def project_velocity(self):
        mass=self.mass_matrix()
        mobility,info1=torch.linalg.solve_ex(mass,self.selector)
        impulse,info2=torch.linalg.solve_ex(mobility[:,self.slide_v],-self.qvel[:,self.slide_v,None])
        self.solve_info.add_((info1!=0).sum().to(torch.int32)+(info2!=0).sum().to(torch.int32))
        self.qvel.add_(torch.bmm(mobility,impulse).squeeze(-1))
    ...
    def step(self, action):
        ...
        with torch.cuda.stream(self.torch_stream),wp.ScopedStream(self.stream):
            ...
            for _ in range(4):
                v=self.qvel[:,self.vi] if self.observer is None else self.observer.qd
                self.ctrl.copy_((.55*(target-self.qpos[:,self.qi])).clamp(-self.limit,self.limit)-.053*v-.0048*torch.tanh(v/.05))
                old_q=self.qpos.clone()
                if self.step_graph is None:mw.step(self.wm,self.wd)
                else:wp.capture_launch(self.step_graph)
                self.project_velocity()
                # Reintegrate from the actual start pose using corrected
                # generalized velocities; do not append a second full step.
                old_q_wp=wp.from_torch(old_q)
                wp.launch(_next_position,dim=(self.count,self.model.njnt),inputs=[self.wm.opt.timestep,self.wm.jnt_type,self.wm.jnt_qposadr,self.wm.jnt_dofadr,old_q_wp,self.wd.qvel,1.],outputs=[self.next_q_wp])
                self.qpos.copy_(self.next_q)
                self.project()
                if self.observer is not None:self.observer.update(self.qpos[:,self.qi],self.qpos[:,self.qa+3:self.qa+7])
            self.last.copy_(action)
    ...
    def velocity_canary(self):
        old=self.qvel.clone()
        caller=torch.cuda.current_stream();self.torch_stream.wait_stream(caller)
        with torch.cuda.stream(self.torch_stream),wp.ScopedStream(self.stream):
            self.qvel.copy_(torch.linspace(-.03,.03,self.model.nv,device='cuda').expand(self.count,-1))
            before=self.qvel.clone();mass=self.mass_matrix().clone()
            self.project_velocity()
            momentum_change=torch.bmm(mass,(self.qvel-before)[:,:,None]).squeeze(-1)
            unconstrained=torch.ones(self.model.nv,dtype=torch.bool,device='cuda');unconstrained[self.slide_v]=False
            residual=self.qvel[:,self.slide_v].abs().max()
            momentum=momentum_change[:,unconstrained].abs().max()
            energy_before=torch.einsum('bi,bij,bj->b',before,mass,before)
            energy_after=torch.einsum('bi,bij,bj->b',self.qvel,mass,self.qvel)
            increase=(energy_after-energy_before).max()
        caller.wait_stream(self.torch_stream)
        result=dict(slide_velocity_max=float(residual),unconstrained_generalized_momentum_change_max=float(momentum),max_energy_increase=float(increase))
        self.qvel.copy_(old)
        assert result['slide_velocity_max']<1e-6 and result['unconstrained_generalized_momentum_change_max']<1e-6 and result['max_energy_increase']<=1e-8,result
        return result
