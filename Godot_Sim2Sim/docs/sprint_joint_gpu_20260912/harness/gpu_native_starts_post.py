...
try:
 ...
 atomic_json(a.out/'completed.json',dict(status='completed',actor_sha256=actor.sha256,ordinary_sha256=ordinary.sha256,feet=a.feet,worlds=n,elapsed_rollout_s=elapsed,parity_max_abs=parity,passes=sum(x['metrics']['success'] for x in candidates),falls=sum(x['metrics']['fell'] for x in candidates),candidate_count=len(candidates),results=results,sampling='post_action_with_final_state_v2',scope='Projected native zero-velocity reset and identical input tapes; different contact solver, no native acceptance substitution'))
