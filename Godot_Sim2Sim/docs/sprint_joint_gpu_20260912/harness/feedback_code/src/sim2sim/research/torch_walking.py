...
class WalkingControl:
    def __init__(self, count, settings, device):
        ...
        self.turn_sign = self.yaw_target.clone()
    ...
    def command(self, requested, pos, quat, velocity, sprint):
        ...
        if gain > 0:
            ...
            tracked = turning & sprint & bool(s.get('walk_sprint_turn_tracking', False))
            sign = torch.where(tracked, requested[:, 2].double().sign(), 0.)
            changed = sign != self.turn_sign
            self.yaw_target = torch.where(changed | (turning & ~tracked), yaw, self.yaw_target)
            error = self.yaw_target - yaw
            correction = (gain * torch.atan2(error.sin(), error.cos())).clamp(-limit, limit)
            tracked_error = torch.atan2(error.sin(), error.cos()).clamp(-limit/gain, limit/gain)
            manual = requested[:, 2].double() + torch.where(tracked, gain*tracked_error, 0.)
            out[:, 2] = torch.where(turning, manual, correction).float()
            self.yaw_target = torch.where(tracked, yaw+tracked_error+requested[:, 2].double()*.02, self.yaw_target)
            self.turn_sign = sign
