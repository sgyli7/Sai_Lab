...
class MotionControl:
    ...
    def command(self,command,state,skill,dt=.02):
        ...
        if skill not in ('walking','roller'):
            self.brake_elapsed=0.;self.was_braking=False;self.walk_has_moved=False;self.walk_path_started=False;self.walk_was_moving=False;self.walk_turn_sign=0;return out
        if skill=='roller' and self.settings.get('heading_hold',False):
            ...
            turn_sign=int(np.sign(float(command[2]))) if (not ordinary and not idle_only and
                self.settings.get('walk_sprint_turn_tracking',False) and abs(float(command[2]))>.05) else 0
            if self.walk_turn_sign and turn_sign!=self.walk_turn_sign:self.target_yaw=yaw
            if abs(float(out[2]))>.05 or (idle_only and moving):
                if turn_sign:
                    if turn_sign!=self.walk_turn_sign:self.target_yaw=yaw
                    gain=float(self.settings['walk_heading_gain']);limit=float(self.settings.get('walk_heading_limit',.3))
                    difference=self.target_yaw-yaw
                    error=float(np.clip(math.atan2(math.sin(difference),math.cos(difference)),-limit/gain,limit/gain))
                    out[2]=float(command[2])+gain*error
                    # Track the next requested heading; clamp lag to prevent windup.
                    self.target_yaw=yaw+error+float(command[2])*dt
                else:self.target_yaw=yaw
