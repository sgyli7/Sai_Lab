...
func command(input: PackedFloat32Array, body: Dictionary, skill: String, dt: float) -> PackedFloat32Array:
	...
	if skill == "roller" and settings.get("heading_hold",false):
		...
		if moving:
			walk_has_moved=true
			walk_idle_elapsed=0.0
		var turn_sign := int(signf(input[2])) if (not ordinary and not idle_only and settings.get("walk_sprint_turn_tracking",false) and absf(input[2])>0.05) else 0
		if walk_turn_sign!=0 and turn_sign!=walk_turn_sign: target_yaw=yaw
		if absf(out[2])>0.05 or (idle_only and moving):
			if turn_sign!=0:
				if turn_sign!=walk_turn_sign: target_yaw=yaw
				var gain := float(settings.walk_heading_gain)
				var limit := float(settings.get("walk_heading_limit",0.3))
				var difference := target_yaw-yaw
				var error := clampf(atan2(sin(difference),cos(difference)),-limit/gain,limit/gain)
				out[2]=float(input[2])+gain*error
				target_yaw=yaw+error+float(input[2])*dt
			else: target_yaw=yaw
