...
p=argparse.ArgumentParser();p.add_argument('--actor',type=Path,required=True);p.add_argument('--out',type=Path,required=True);p.add_argument('--seeds',type=int,default=16);a=p.parse_args()
...
for name,skill in [('Walk_Godot.onnx','walking'),('Sprint_Godot.onnx','sprint')]:
 source=a.actor if skill=='sprint' else Path('results/sprint_20260912/runs/s05_native_handoff/final.onnx')
...
cases=write_cases(a.out/'cases',range(925000,925000+a.seeds),.3,paired=True,control=control)
...
result=dict(errors=s['errors'],candidate_count=len(c),candidate_pass=sum(e['task_metrics']['success'] for e in c),candidate_falls=sum(e['task_metrics']['fell'] for e in c),accepted=accept['accepted'] and minimal['task_metrics']['success'],mini=minimal['task_metrics'],models=s['models'],comparisons=[{k:v for k,v in x.items() if k!='pairs'} for x in accept['comparisons']])
