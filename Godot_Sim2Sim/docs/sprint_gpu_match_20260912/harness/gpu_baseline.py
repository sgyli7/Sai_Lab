...
p=argparse.ArgumentParser();p.add_argument('--contact',choices=['source','two_tick'],required=True);p.add_argument('--feet',choices=['source','jolt'],default='source');a=p.parse_args()
r=Path('results/sprint_gpu_match_20260912');out=r/('baseline_'+a.contact+('_jolt_feet' if a.feet=='jolt' else ''));out.mkdir(exist_ok=False)
...
world=GpuWorld(n,settings,contact=a.contact,feet=a.feet);actor=TorchAnchor('results/sprint_20260912/runs/s05_native_handoff/final.onnx').cuda().eval()
