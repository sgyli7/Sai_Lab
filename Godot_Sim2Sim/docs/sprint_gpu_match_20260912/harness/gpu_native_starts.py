...
p=argparse.ArgumentParser();p.add_argument('--actor',type=Path,required=True);p.add_argument('--feet',choices=['source','jolt'],required=True);p.add_argument('--out',type=Path,required=True);a=p.parse_args()
a.out.mkdir(exist_ok=False,parents=True)
r=Path('results/sprint_gpu_match_20260912');source=Path('results/sprint_20260912/runs/s05_native_handoff/final.onnx')
summary=json.loads((r/'native_s05/suite/summary.json').read_text());assert summary['errors']==0
...
inputs=np.concatenate([np.random.default_rng(925014).normal(0,.5,(256,61)).astype(np.float32),np.array([x['obs'] for x in first],np.float32)])
