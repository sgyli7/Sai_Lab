...
def audit_behavior(data, policy, anchor, exported):
    ...
    with torch.no_grad():
        ...
        assert parity < 1e-5 and ordinary_error < 1e-5, (parity,ordinary_error)
    ...
    return flat,dict(policy_export_max_abs=parity,ordinary_max_abs=ordinary_error,
...
def main():
    ...
    config=dict(seed=a.seed,iterations=a.iterations,seeds_per_batch=a.seeds_per_batch,
                ...
                ordinary_controller='frozen S05',actor_mask='stochastic sprint transitions before first fall',
                seed_range_note='Fresh training resets starting 955200 + iteration*8; no final seeds',
    ...
    for iteration in range(1,a.iterations+1):
        ...
        rollouts,deployment=collect(project,actor,out/'rollouts',
                                    range(955200+iteration*8,955200+iteration*8+a.seeds_per_batch),
                                    95520000+iteration*100)
