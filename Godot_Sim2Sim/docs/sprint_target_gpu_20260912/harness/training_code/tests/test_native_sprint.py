...
class NativeSprintDatasetTests(unittest.TestCase):
    def fixture(self, directory, *, heights=(.125,.125,.125), velocities=(0.,1.,0.), sigma=.02):
        root=Path(directory)
        rows=[]
        ...
        trace.write_text(json.dumps(dict(rows=rows,summary=dict(training_exploration=sigma,models={},final_raw=raw))))
        case.write_text(json.dumps(dict(case='sprint_shift_first')))
        rollouts=dict(rows=[dict(trace=str(trace),case=str(case),steps=len(rows),sigma=sigma)])
