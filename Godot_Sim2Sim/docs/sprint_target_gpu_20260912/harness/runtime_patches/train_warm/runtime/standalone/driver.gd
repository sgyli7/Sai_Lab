...
func _ready() -> void:
	...
	if not is_finite(sample_after) or sample_after<0.0 or sample_after>25.0:
	...
	_reset_controller()
	bank["sprint"] = TrainingPolicy.new(bank["sprint"],sample_std)
	ready_to_run = true
...
func _finish() -> void:
	ready_to_run = false
	var result := {"training_exploration":sample_std,"training_exploration_after":sample_after,"schema_version":1,"mode":session.mode,"steps":session.steps,
