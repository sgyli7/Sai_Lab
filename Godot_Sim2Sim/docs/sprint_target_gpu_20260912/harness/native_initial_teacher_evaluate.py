...
assert a.iteration in json.loads((r/'initial_teacher_protocol.json').read_text())['endpoints']
assert json.loads((r/'train_initial_teacher/completed.json').read_text())['completed']
actor=r/'train_initial_teacher'/f'iteration_{a.iteration:03d}'/'actor.onnx';out=r/f'native_initial_teacher_{a.iteration:03d}'
